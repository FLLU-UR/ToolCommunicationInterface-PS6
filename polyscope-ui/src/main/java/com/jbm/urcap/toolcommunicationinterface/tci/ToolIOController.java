...
public class ToolIOController implements ToolIOInterfaceController{
	 ...
	 */
	private static final BaudRate 		BAUDRATE = 		BaudRate.BAUD_9600;
	...
	public void onControlGranted(ToolIOInterfaceControlEvent event) {
		...
		powerOnTool();
		setupTCI(BAUDRATE, PARITY, STOPBITS);
	}
	...
	public void setupTCI(BaudRate baudrate, Parity parity, StopBits stopbits) {
		if(capabilityManager.hasCapability(COMMUNICATION_INTERFACE_MODE)) {
			...
			System.out.print("setupTCI:");
			System.out.printf(baudrate.toString(), parity.toString(), stopbits.toString());
		}
	}
